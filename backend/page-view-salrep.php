...
if (isset($_REQUEST['custsalrep'])) {
    $cid = $_REQUEST['getscid'];

    $dd = date("d-m-Y");
    
    include "../connection.php";
    $ll = "SELECT * FROM sale WHERE custid = '$cid' ";
    $lexe = mysqli_query($conn,$ll);
    $cpy = mysqli_query($conn,$ll);
    if (mysqli_num_rows($lexe) == 0) {
        $ll = "SELECT * FROM sale WHERE custid = '$cid' LIMIT 0";
        $lexe = mysqli_query($conn,$ll);
        $cpy = mysqli_query($conn,$ll);
    }
...
if (isset($_REQUEST['datesalrep'])) {
    $sdate = $_REQUEST['getsdate'];

    $dd = $sdate;
    
    include "../connection.php";
    $ll = "SELECT * FROM sale WHERE saldate = '$sdate' ";
    $lexe = mysqli_query($conn,$ll);
    $cpy = mysqli_query($conn,$ll);
    if (mysqli_num_rows($lexe) == 0) {
        $ll = "SELECT * FROM sale WHERE saldate = '$sdate' LIMIT 0";
        $lexe = mysqli_query($conn,$ll);
        $cpy = mysqli_query($conn,$ll);
    }
...
if (isset($_REQUEST['rdatesalrep'])) {
    $stdate = $_REQUEST['sdatestart'];
    $endate = $_REQUEST['sdateend'];

    $dd = $stdate.' --- '.$endate;
    
    include "../connection.php";
    $ll = "SELECT * FROM sale WHERE saldate BETWEEN  '$stdate' AND '$endate' ";
    $lexe = mysqli_query($conn,$ll);
    $cpy = mysqli_query($conn,$ll);
    if (mysqli_num_rows($lexe) == 0) {
        $ll = "SELECT * FROM sale WHERE saldate BETWEEN  '$stdate' AND '$endate' LIMIT 0";
        $lexe = mysqli_query($conn,$ll);
        $cpy = mysqli_query($conn,$ll);
    }
}
...
<html lang="en">
  ...
  <script type="text/javascript">
    ...
    function print_content(){
      ...
      a.document.write('<body><center><h2><u>Sale Report Details</u></h2>Date - <?php echo $dd; ?><br><br><table border="1">');
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">View Sale Report</h4>
                        </div>
                    ...
                    <div class="card-body">
                            <div class="col-md-12">
                                <div class="row mb-4">
                                    ...
                                    <h4 style="margin-left: 430px;" class="h5 text-black"><b><u>Sale Report Details</u></b></h4>
                                </div>
                                 ...
                                 <tbody>
                                    ...
                                    if (mysqli_num_rows($lexe) > 0) {
                                       while ($listdata = mysqli_fetch_array($lexe)) {
                                          $putc = "SELECT name,phone,address FROM customers WHERE cid = '$listdata[3]' ";
                                    ...
                                    <?php
                                            $stotal = $stotal + floatval($listdata[5]);
                                            $spaid = $spaid + floatval($listdata[6]);
                                            $sdue = $sdue + floatval($listdata[7]);
                                        }
                                    }
                                        ?>
                                 </tbody>
