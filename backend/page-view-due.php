...
if (isset($_REQUEST['dateduerep'])) {
    $sdate = $_REQUEST['getduedate'];

    $dd = $sdate;
    
    include "../connection.php";
    $ll = "SELECT * FROM payment WHERE paydate = '$sdate' AND balance != 0.00 ";
    $lexe = mysqli_query($conn,$ll);
    $cpy = mysqli_query($conn,$ll);
    if (mysqli_num_rows($lexe) == 0) {
        $ll = "SELECT * FROM payment WHERE paydate = '$sdate' AND balance != 0.00LIMIT 0";
        $lexe = mysqli_query($conn,$ll);
        $cpy = mysqli_query($conn,$ll);
    }
...
if (isset($_REQUEST['rdateduerep'])) {
    $stdate = $_REQUEST['duedatestart'];
    $endate = $_REQUEST['duedateend'];

    $dd = $stdate.' --- '.$endate;
    
    include "../connection.php";
    $ll = "SELECT * FROM payment WHERE balance != 0.00 AND paydate BETWEEN '$stdate' AND '$endate' ";
    $lexe = mysqli_query($conn,$ll);
    $cpy = mysqli_query($conn,$ll);
    if (mysqli_num_rows($lexe) == 0) {
        $ll = "SELECT * FROM payment WHERE balance != 0.00 AND paydate BETWEEN '$stdate' AND '$endate' LIMIT 0";
        $lexe = mysqli_query($conn,$ll);
        $cpy = mysqli_query($conn,$ll);
    }
}
...
<html lang="en">
  <head>
    <meta charset="utf-8">
      ...
      <title>SHOP | Payment Due Report</title>
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">View Payment Due Report</h4>
                        </div>
                    ...
                    <div class="card-body">
                            <div class="col-md-12">
                                <div class="row mb-4">
                                    ...
                                    <h4 style="margin-left: 430px;" class="h5 text-black"><b><u>Payment Due Report Details</u></b></h4>
                                </div>
                                <table id="print_area" class="text-center w-100" border="2" role="grid" aria-describedby="user-list-page-info">
                                 ...
                                 <tbody>
                                    <?php 
                                    $ctotal = $cpaid = $cdue = 0.00;
                                    $dtotal = $dpaid = $ddue = 0.00;
                                    $type = $stst = "";
                                    if (mysqli_num_rows($lexe) > 0) {
