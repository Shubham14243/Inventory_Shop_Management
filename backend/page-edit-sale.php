...
if (!isset($_SESSION['sdateup']) || !isset($_SESSION['sidup']) || !isset($_SESSION['scidup']) || !isset($_SESSION['scartno'])) {
  header("location:page-list-sale.php");
}

$bsdate = $_SESSION['sdateup'];
$custid = $_SESSION['scidup'];
$prid = $_SESSION['sidup'];
$cartno = $_SESSION['scartno'];
...
if (mysqli_num_rows($lexe1) > 0) {
    while ($getx = mysqli_fetch_array($lexe1)) {
        if ($getx[0] == $bsdate && $getx[1] == $custid && $getx[2] == $cartno) {
            $ll = "SELECT * FROM billsale WHERE bsdate = '$bsdate' AND custid = '$custid' AND cartno = '$cartno' ";
            $lexe = mysqli_query($conn,$ll);
            $flag = 1;
            break;
        }
...
if (isset($_REQUEST['modput'])) {
    ...
    $chk = "SELECT * FROM billsale WHERE  bsdate = '$bsdate' AND custid = '$custid' AND cartno = '$cartno' AND proid = '$gid' ";
    $exchk = mysqli_query($conn,$chk);
    if (mysqli_num_rows($exchk) == 0) {
        ...
        echo '<script>alert("Products Already Added!");</script>';
    }   
}
    ...
    if (mysqli_num_rows($bsexe) > 0) {
        while ($getq = mysqli_fetch_array($bsexe)) {
            ...
            if (intval($pdat[0]) > intval($getq[1])) {
               ...
               echo '<script>alert("Only '.$pdat[0].' Units Of '.$pdat[1].' is Left!");</script>';
               $c = 0 ;
            }
        }
    }
...
if (isset($_REQUEST['dele'])) {
    ...
    $getd = "SELECT proid,quantity FROM billsale WHERE bsid = '$getid' ";
    $exgetd = mysqli_query($conn,$getd);
    $qtydata = mysqli_fetch_array($exgetd);

    $selp = "SELECT quantity FROM products WHERE pid = '$qtydata[0]' ";
    $exselp = mysqli_query($conn,$selp);
    $pqty = mysqli_fetch_array($exselp);

    $putqty = intval($pqty[0]) + intval($qtydata[1]);

    $insp = "UPDATE products SET quantity = '$putqty' WHERE pid = '$qtydata[0]' ";
    $inspex = mysqli_query($conn,$insp);
...
if (isset($_REQUEST['updsale'])) {
   ...
   $userdata1 = mysqli_fetch_array($exe1);
    ...
    if ($bpupd) {
        $sqsq = "UPDATE sale SET salstatus = '$salstatus' , total = '$total' , paid = '$paid' , balance = '$balance' , paystatus = '$paystatus' , mode = '$paymode' , biller = '$biller' WHERE salid = '$saleid' ";
        $execute = mysqli_query($conn,$sqsq);
        if ($execute) {
            $inspay = "UPDATE payment SET amount = '$total' , paid = '$paid' , balance = '$balance' , paydate = '$bsdate' , mode = '$paymode' WHERE recid = '$saleid' AND type = 'Customer' ";
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                $gg = "SELECT * FROM sale WHERE cartno = '$cartno' AND saldate = '$bsdate' AND custid = '$custid' ";
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <?php include "../connection.php";
                                        $gg = "SELECT * FROM customers WHERE cid = '$custid' ";
                                        $ee = mysqli_query($conn,$gg);
                                        $row = mysqli_fetch_array($ee);
                                     ?>
                                    <div class="col-md-8">
                                        <h4 class="h5 text-black"><?php echo $row[1]; ?> </h4>
                                ...
                                <table id="user-list-table" class="table table-striped dataTable mt-4 text-center" role="grid" aria-describedby="user-list-page-info">
                                 ...
                                 <tbody>
                                    ...
                                    <tr>
                                       ...
                                       <td><input size="7" name="discount" type="text" value="0.00"></td>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="selectpicker form-control" id="purstatus" data-style="py-0"  name="salstatus" required>
                                ...
                                <div class="col-md-6" style="display:none;">                      
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" placeholder="Sale ID" value="<?php echo $pur[0]; ?>" id="saleid" name="saleid" required>
                                        <div class="help-block with-errors"></div>
                                    </div>
                                </div>
                            ...
                            <button type="submit" name="updsale" class="btn btn-primary mr-2">Update Sale</button>
