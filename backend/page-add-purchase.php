...
if (!isset($_SESSION['bpdate']) || !isset($_SESSION['bpsid']) || !isset($_SESSION['cart'])) {
  header("location:page-list-purchase.php");
}

$bpdate = $_SESSION['bpdate'];
$bpsid = $_SESSION['bpsid'];
$cartno = $_SESSION['cart'];
...
if (mysqli_num_rows($lexe1) > 0) {
    ...
    if ($i == 0) {
        $ll = "SELECT * FROM billpurchase LIMIT 0";
        $lexe = mysqli_query($conn,$ll);
    }
}
if (mysqli_num_rows($lexe1) ==  0) {
...
if (isset($_REQUEST['addpur'])) {
    ...
    $c=0;
    $bpins = "SELECT bppid,quantity,cost,mrp,tax,discount FROM billpurchase WHERE bpsid = '$bpsid' AND bpdate = '$bpdate' AND cartno = '$cartno'";
    ...
    if (mysqli_num_rows($bpexe) > 0) {
        while ($getq = mysqli_fetch_array($bpexe)) {
            ...
            $setqty = intval($getq[1]) + intval($pdat[0]);
    ...
    if ($c == 1) {
        ...
        if ($bpupd) {
            $sqsq = "INSERT INTO purchase(cartno, purdate, supplier, gst, purstatus, total, paid, balance, paystatus, mode) VALUES('$cartno','$bpdate','$bpsid','$ddd[6]','$purstatus','$total','$paid','$balance','$paystatus','$paymode')";
            $execute = mysqli_query($conn,$sqsq);
            if ($execute) {
                $getpay = "SELECT purid FROM purchase WHERE supplier = '$bpsid' AND purdate = '$bpdate' AND cartno = '$cartno' ";
                $exee = mysqli_query($conn,$getpay);
                $paydata = mysqli_fetch_array($exee);
                $type = "Supplier";
                $inspay = "INSERT INTO payment(type, typeid, amount, paid, balance, paydate, mode, recid) VALUES('$type', '$bpsid', '$total', '$paid', '$balance', '$bpdate', '$paymode', '$paydata[0]' )";
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <button type="submit" name="addpur" class="btn btn-primary mr-2">Add Purchase</button>
