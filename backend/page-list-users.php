...
$ll = "SELECT * FROM users ORDER BY uid DESC";
$lexe = mysqli_query($conn,$ll);
if (mysqli_num_rows($lexe) == 0) {
   $ll = "SELECT * FROM users ORDER BY uid DESC LIMIT 0";
   $lexe = mysqli_query($conn,$ll);
}
...
if (isset($_REQUEST['getsch'])) {
   ...
   include "../connection.php";
   if ($sch != "") {
      $ll = "SELECT * FROM users WHERE name = '$sch' ";
      $lexe = mysqli_query($conn,$ll);
   }else{
      $ll = "SELECT * FROM users";
      $lexe = mysqli_query($conn,$ll);
   }
}
...
if (isset($_REQUEST['upd'])) {
   ...
   header("location:page-edit-users.php");
}

if (isset($_REQUEST['dele'])) {
   include "../connection.php";
   $gdid = $_REQUEST['getid'];
   $sql2 = "DELETE FROM users WHERE uid ='$gdid' ";
...
<html lang="en">
  ...
  <script type="text/javascript">
    ...
    function print_content(){
      ...
      a.document.write('<body><center><h2>Users List</h2><br><table border="1"><thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Email</th><th>GST No</th><th>Address</th><th>Business Name</th><th>Role</th><th>Join Date</th></tr></thead>');
      a.document.write(divContents);
      a.document.write('</table></center></body></html>');
      a.document.close();
      a.print();
      window.location.replace('page-list-users.php');
    }
         ...
         <div class="card">
            <div class="card-header d-flex justify-content-between">
               <div class="header-title">
                  ...
                  <a style="position: absolute;right: 2%;top:3%;" class="btn btn-primary" href="page-add-users.php">Add User</a>
               </div>
            </div>
            <div class="card-body">
